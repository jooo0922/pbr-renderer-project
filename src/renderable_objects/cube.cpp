...
Cube::Cube()
{
  ...
  mesh = std::make_unique<Mesh<SimpleVertexData>>("cube", vertices, indices);
}
...
void Cube::generateVertexData(std::vector<SimpleVertexData> &vertices, std::vector<unsigned int> &indices) const
{
  // Cube 정점 데이터 추가
  vertices = {
